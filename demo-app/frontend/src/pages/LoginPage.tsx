...
// CRUST SDK is loaded as a script tag in index.html and available on window
declare global {
  interface Window {
    CRUST: {
...
type PageState = 'idle' | 'collecting' | 'challenge' | 'submitting' | 'success' | 'blocked';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let jwt: string;
    try {
      // SDK collects signals passively then returns a JWT.
      // If SOFT_CHALLENGE, it renders ToppingsChallenge inline and resolves
      // only after the user completes it successfully.
      ...
      <div style={card}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button
            style={{
              ...btnPrimary,
              opacity: pageState === 'idle' || pageState === 'blocked' ? 1 : 0.7,
            }}
            type="submit"
            disabled={pageState !== 'idle' && pageState !== 'blocked'}
          >
            ...
            {pageState === 'blocked'     && 'Try again'}
            {pageState === 'challenge'   && '🍕 Complete challenge…'}
          </button>
        </form>
...
const btnPrimary: React.CSSProperties = {
  height:        '40px',
  background:    'var(--button-primary-bg, #080808)',
...
const errorBox: React.CSSProperties = {
  ...
  color:        'var(--state-error-fg, #b91c1c)',
  border:       '1px solid var(--state-error-border, #ef4444)',
  borderRadius: '8px',
  fontSize:     '13px',
};
